...
export default function FormEditProduct() {
    const { id } = useParams();
    const { product } = useProductManagementStore();
    const { categories } = useCategoryManagementStore();

    const selectedProduct = product?.products || {}; // Handle undefined product
    const [selectedCategory, setSelectedCategory] = useState(selectedProduct.category_id || ''); // Default to empty string
    ...
    const availableSubCategories = allCategory[selectedCategory - 1]?.subcategories || [];
